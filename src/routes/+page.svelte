...
<div class="flex justify-between">
  <column>
    <div class="mt-4">type relay url: <input bind:value={relayUrl} on:input={tryConnect} /></div>
    <div class="mt-2 text-right">
      ...
      {#if relay}
        <div class="mt-4">type group id: <input bind:value={groupId} on:input={encode} /></div>
      {/if}
      <div class="mt-4">
        {#if relay && groupId}
          <button
  ...
  <column>
    <div class="mt-4 flex items-center">
      type a group code: <textarea class="ml-2 h-48" bind:value={naddr} on:input={parse} />
    </div>
  </column>
</div>
